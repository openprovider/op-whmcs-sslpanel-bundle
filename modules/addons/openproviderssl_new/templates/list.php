...
<ul>
    <?php foreach ($view['userlist'] as $user): ?>
        <li>
            <a href="<?php echo $view['global']['mod_action_url'] ?>&user=<?php echo $user['user'] ?>&acc=<?php echo $user['acc'] ?>"><?php echo $user['user'] ?></a>
        </li>
    <?php endforeach; ?>
</ul>
    ...
    <table width="100%" cellspacing="1" cellpadding="3" border="0" class="datatable" id="sortabletbl1">
        ...
        <?php foreach ($view['items'] as $item): ?>
            <tr>
                ...
                <td><?php if (!empty($item['whmcs'])): ?><a target="_blank"
                                                            href="./clientsservices.php?userid=&id=<?php echo $item['whmcs']['serviceid'] ?>"><?php echo $item['whmcs']['serviceid']; ?></a><?php else: ?>-<?php endif; ?>
                </td>
